...
var JsonSchema = new function() {
	...
	function Schema4Schema(aJsonObject) {
		...
		for(k in keys) {
			...
			if (propertyKey == '$schema') {
				schema.set({dollarschema:propertyValue});
			} else if(propertyKey == 'required') {
				schema.set({required:propertyValue});
			} else if(propertyKey == 'title') {
				schema.set({title:propertyValue});
			} else if(propertyKey == 'name') {
				schema.set({name:propertyValue});
			} else if(propertyKey == 'description') {
				schema.set({description:propertyValue});
			} else if(propertyKey == 'minimum') {
				schema.set({minimum:propertyValue});
			} else if(propertyKey == 'maximum') {
				schema.set({maximum:propertyValue});
			} else if(propertyKey == 'minitems') {
				schema.set({minitems:propertyValue});
			} else if(propertyKey == 'maxitems') {
				schema.set({maxitems:propertyValue});
				...
				for(l in nestedKeys) {
					...
					var nestedSchemaPair = new SchemaPair({key:nestedPropertyKey, schema:nestedSchema});
				...
				if (propertyValueType == TypeEnum.OBJECT) {
					...
					var nestedSchemaPair = new SchemaPair({schema:Schema4Schema(propertyValue)});
					...
					for(m in nestedKeys) {
						...
						var nestedSchema = Schema4Schema(nestedPropertyValue);
						var nestedSchemaPair = new SchemaPair({schema:nestedSchema});
						schema.addItem(nestedSchemaPair);
					}
				}
				...
				if (propertyValueType == TypeEnum.ARRAY) {
					...
					for(n in nestedKeys) {
						...
						var nestedPropertyValue = propertyValue[nestedPropertyKey];
						var type = new Type({t:nestedPropertyValue});
						schema.addType(type);
					}
				} else if (propertyValueType == TypeEnum.STRING) {
					var type = new Type({t:propertyValue});
					schema.addType(type);
				}
			}
		}
	...
	function Schema4Object(aJsonObject) {
		...
		var objectType = new Type({t:TypeEnum.OBJECT});
		var schema = new Schema();
		...
		for(k in keys) {
			...
			if (propertyValueType == TypeEnum.STRING || 
				propertyValueType == TypeEnum.NUMBER || 
				propertyValueType == TypeEnum.BOOLEAN) {

				var type = new Type({t:propertyValueType});
				var propertySchema = new Schema();
				propertySchema.addType(type);

				var propertySchemaPair = new SchemaPair({key:propertyKey, schema:propertySchema});
				...
				var propertySchemaPair = new SchemaPair({key:propertyKey, schema:itemSchema});
				schema.addProperty(propertySchemaPair);
			}
		}
		return schema;
	...
	function Schema4Array(aJsonArray) {
		...
		var schema = new Schema();
		var type = new Type({t:TypeEnum.ARRAY});
		schema.addType(type);
		...
		for(k in keys) {
			...
			itemSchemaPair = new SchemaPair({schema:itemSchema});
       ...
       */
		if (doTupleTyping) {
			...
			if (schemaPairArray[0]) {
				...
				schema.addItem(new SchemaPair({schema:new Schema()}));
			}
		}
		return schema;
	}
	...
	this.GenerateSchema = function() {
		...
		var jsonString = $('#UserInput textarea').val().replace(/\s+/g,'');
		var inputType = $('#UserInput input:[name=InputType]:checked').val();
		
		var jsonObject;

		if (jsonString == '') {
			return new Schema({dollarschema:schemaVersion});
		}
		...
		if (inputType == 'schema') {
			...
			schema.set({dollarschema:schemaVersion});
		}
